...
Full probes check Cloudflare/GStatic, YouTube, ChatGPT, the OpenAI API, Telegram Web, Telegram MTProto, Instagram, and a bounded speed download. Ordinary VLESS becomes UDP-eligible only after two fresh QUIC/TLS handshakes with YouTube over a SOCKS5 UDP association. Standard `xtls-rprx-vision` intentionally rejects UDP/443, so it is checked with DNS-over-UDP through the same SOCKS5 association; `xtls-rprx-vision-udp443` keeps the QUIC check. The YouTube gate requires a successful response from `https://www.youtube.com/generate_204`. The Instagram gate requires a response below 400 from `https://www.instagram.com/`. The ChatGPT gate accepts the origin response without following redirects: any 2xx/3xx or authentic origin 403/429. The OpenAI gate calls `https://api.openai.com/v1/models` and accepts 401, 204, or 200. Telegram requires both Web availability and a valid MTProto response.
...
For VLESS, QoE repeats the matching UDP check concurrently with the TCP measurement: DNS-over-UDP for standard `xtls-rprx-vision`, QUIC for other flows. Two consecutive failures remove only `UDPQualified`, retaining TCP score, availability, and freshness; three consecutive successes restore UDP eligibility. Every transition triggers immediate UDP replanning without evacuating working TCP or reconnecting the WireGuard peer. This separate hysteresis loop prevents switches caused by a single lost packet.
